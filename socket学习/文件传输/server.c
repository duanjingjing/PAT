/*************************************************************************
	...
	> Created Time: Tue 26 Aug 2014 04:05:18 AM PDT
 ************************************************************************/
...
int main(int argc,char *argv[]){  //exe ip port
	...
	if(-1==setsockopt(fd_server,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(int))){
		perror("setsockopt");
		close(fd_server);
		exit(-1);
	}
	...
	if(-1==bind(fd_server,(struct sockaddr*)&server_addr,len)){
		perror("bind");
		close(fd_server);
		exit(-1);
	}
	...
	while(fd_client=accept(fd_server,(struct sockaddr*)&client_addr,&len)){
